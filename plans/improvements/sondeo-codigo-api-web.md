...
**Fase 3 — Endurecimiento medio: parcial (2026-07-11), sin commitear.** De seguridad (S4–S9, S11), correctness (B7–B12), DRY (D1–D3) y UI (Q1), se hicieron los ítems bien acotados; se dejaron explícitamente pendientes los que son decisiones de producto/política de seguridad o refactors estructurales grandes (ver "Pendiente" abajo).

Hecho:

- **B12** — la causa real no era `require_attributes` (como decía el sondeo original), sino `AegisManager.assert_document_ownership`: lanzaba el `DocumentError` genérico (500) en vez de `DocumentNotFoundError` (404). Un solo cambio en `managers.py`; se quitó el `xfail(strict=True)` del test, que ahora pasa de verdad.
- **B8** — `get_scan_status()` devolvía `"completed"` (vocabulario de `TaskStatus`) en su fallback a BD en vez de `"finished"` (vocabulario de `ScanStatus`, lo que persiste `scan.status`). Ahora usa `ScanStatus.FINISHED.value` en ambos sitios.
- **S6** — `GET /acheron/generate-password` ahora exige `@require_oauth_token` (no tenía ningún consumidor sin sesión). `GET /system/say-hello` se deja público a propósito: es un health check convencional (headers/balanceadores no llevan JWT); forzar auth ahí rompería su propósito, no es un bug.
- **B10** (`irisStore.js`) — el `setInterval` async de `startPolling` no esperaba a que la petición anterior terminara; con una respuesta lenta (>2s) podían dispararse sondeos solapados. Reescrito como `setTimeout` re-encadenado (mismo idioma que traceroute/Themis), con guarda contra reprogramar tras un `stopPolling()` en vuelo. Verificado en navegador real con respuestas de 2.5s: `maxConcurrent` pasó de posible >1 a 1 siempre.
- **B9** (`ThemisView.vue`) — `handlePreviewPdf` esperaba 600ms fijos y refrescaba, asumiendo que la generación (encolada, asíncrona) siempre terminaba a tiempo. Nuevo `themisStore.waitForDocument(scanId)` sondea `/themis/document-status` hasta `done`/`error` (tope 20 intentos × 1.5s). Verificado en navegador real: con una generación simulada de >600ms, el viejo código habría refrescado antes de tiempo; el nuevo espera lo que haga falta y se acota si nunca termina.
- **B7** (`scan.py::_execute_scan`) — `scan.target`/`scan.scan_type` se leían de una instancia ORM *detached* fuera de su `UnitOfWork`, funcionando solo porque `expire_on_commit=False` lo permite implícitamente. Ahora se extraen a variables locales dentro del bloque que cargó la instancia. (La otra mitad del hallazgo original — `_log_to_csv(fresh_scan=None)` causando `AttributeError` — se verificó que ya no aplica: esa función re-consulta su propio `scan` y ni siquiera usa el parámetro.)
- **S5** — `ProxyFix` (Werkzeug) añadido en `create_app()`: sin él, `request.remote_addr` veía la IP del contenedor de nginx (confirmado — la app corre detrás de `web/nginx.conf`), no la del cliente, afectando rate limiting y logs de auditoría por IP. `nginx.conf` actualizado para reenviar `X-Forwarded-For/-Proto/-Host` (antes solo mandaba `X-Real-IP`, que `ProxyFix` no lee).
- **S4** — el rate limiter usaba `storage_uri="memory://"` (contador por-proceso: con varios workers de gunicorn, el límite real se multiplica y se resetea en cada reinicio). Ahora `create_app()` fija `app.config["RATELIMIT_STORAGE_URI"]` a Redis (reutilizando `CR.get_redis_config()`) antes de `limiter.init_app()`, con `in_memory_fallback_enabled=True` para no tumbar el rate limiting si Redis cae. (Nota técnica: no se pudo resolver esto dentro de `shared/_endpoints.py` directamente — importar `config_reading` ahí en tiempo de carga del módulo crea un ciclo `shared → system → users → shared`; por eso la config se inyecta desde `run.py`, que es el entrypoint y no tiene ese problema.)
- **D3 + B11** — el patrón `const data = await res?.json().catch(() => ({}))` seguido de `data.error_description || data.message` estaba repetido ~28 veces en 8 stores; si `apiFetch` devolvía `null` (sesión caída o error de red), `res?.json()` cortocircuitaba a `undefined` y `data.message` lanzaba un `TypeError` silencioso (atrapado por el `try/catch` exterior, ocultando el mensaje real). Nuevo `apiError(res, fallback)` en `useApi.js` centraliza esto. **Hallazgo real durante la migración automatizada**: en 9 sitios de `themisStore.js` (y 2 de `aegisStore.js`) la variable `data` también se usaba en el camino de éxito (p. ej. `data.scanId`, `data.folderId`) — un primer intento de migración por regex la eliminó sin darse cuenta, dejando `ReferenceError` latentes que el build de Vite no detecta (no hay type-check). Se revisó cada uno de los ~28 sitios migrados y se restauró `const data = await res.json()` donde hacía falta; verificado ejecutando las funciones reales en el navegador (fetch mockeado) para los casos de éxito Y de error, no solo con `npm run build`.
- **S8** — `JWT_ALGORITHM` se leía de config/env sin validar contra nada; un typo o un `"none"` mal puesto rompería la verificación de tokens. Nueva allowlist (`HS256`/`HS384`/`HS512`, la familia simétrica que corresponde a firmar con un único secreto) en `config_reading.get_oauth_config()`.

**Tests Fase 3:** suite completa verde (WSL). Nuevos/actualizados: `test_aegis.py` (xfail de B12 retirado), `tests/unit/test_config_reading.py` (allowlist S8), `test_themis.py` (B8), `test_acheron.py` (S6, auth requerida). B10/B9/D3 se verificaron con ejecución real en el navegador (Playwright vía la herramienta de preview), no solo con build — el bug de D3 descrito arriba solo se detectó así.

**Pendiente (decisiones que requieren una llamada explícita, no solo "fix directo"):**
- **S7** — `PUT /system` reescribe toda la config (incluye `security.argon2`, `areLocalIpsAllowed`, etc.) con solo `require_role(ADMIN)`, no root. Restringirlo a root es un cambio de comportamiento real para cualquier admin existente que hoy dependa de editar config — decisión de producto, no un bug puro.
- **S9** — el rol se lee del claim JWT y nunca se revalida contra BD (`require_role`); un cambio de rol no revoca tokens ya emitidos (ventana ≤30 min). Hoy no explotable (no existe endpoint de cambio de rol). Arreglarlo significa una query a BD por request autenticado — trade-off de performance que vale la pena discutir, no aplicar a ciegas.
- **S11** — `currentPassword` se recoge en `ProfileView.vue` pero nunca se envía ni se revalida en servidor al cambiar contraseña. Cerrarlo bien requiere schema nuevo (`currentPassword` en el body) + verificación server-side — pequeño pero toca el flujo de autenticación, mejor con luz verde explícita.
- **D1** — `_generate_pdf_async`/`_update_document_status` duplicados casi literalmente entre `themis/managers/reports.py` e `iris/managers.py`. Extraer una base común es un refactor estructural (mover código entre módulos), no un fix de una función.
- **D2** — el patrón `get→exists→remove→delete fila` para borrar documentos está reimplementado en `reports.py` (Themis), `iris/managers.py`, y dentro de `ScanManager.delete_scan`. Mismo tipo de refactor que D1.
- **Q1** — no hay estado de error distinto del estado vacío en las listas de los stores (fetch fallido → lista vacía → "no hay nada que mostrar", en vez de "no se pudo cargar"). Toca UI en varios componentes (Themis/Iris/Aegis/Queue), no solo stores — más una mejora de UX transversal que un bug puntual.

**Hallazgo nuevo (no estaba en el sondeo original):** `web/nginx.conf` enruta `/sentinel/` (nombre viejo del módulo, ahora `themis`) y no tiene ninguna ruta para `/iris/` — en el despliegue dockerizado con este nginx, Iris sería inalcanzable y Themis solo funcionaría si algo más lo redirige. No se tocó porque es un cambio de infraestructura de despliegue que merece verificación explícita (posiblemente ya compensado por otro mecanismo no visible en el repo), no un fix silencioso de una sola línea.
...
3. 🟡 **Fase 3 — Endurecimiento medio (🟠):** seguridad (S4–S9, S11), correctness restante (B7–B12), DRY de alto valor (D1–D3), estado de error en UI (Q1) y tests de Themis (T1). **Parcial 2026-07-11**: B7/B8/B9/B10/B11/B12/S4/S5/S6/S8/D3 hechos; S7/S9/S11/D1/D2/Q1 pendientes por ser decisiones de producto o refactors grandes (ver Progreso de ejecución arriba). T1 sigue igual que antes de esta fase.
