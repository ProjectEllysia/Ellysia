...
```bash
npm install
npm run dev            # dev server on :5173; proxies /oauth,/sentinel,/aegis,/users,/system,/acheron,/iris → Flask :5000 (see vite.config.js)
npm run build
npm run test:acheron   # crypto interop + CRUD tests for the Acheron vault client (node, in test/)
```

## Layout (`src/`)
- `views/` — one component per route (Sentinel, Iris, Aegis, Acheron, Users, Config, Profile, Queue, Login, Landing).
- `stores/` — Pinia stores, one per domain (`authStore`, `sentinelStore`, `mfaStore`, ...).
- `components/` — grouped by feature (`sentinel/`, `aegis/`, `iris/`, `acheron/`, `shared/`, ...).
