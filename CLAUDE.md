...
- The suite is sealed off from the network: `tests/conftest.py` makes every Redis operation fail instantly (`_redis_always_unavailable`) and refuses any `socket.connect`/reverse DNS outside loopback (`_no_outbound_sockets`). A test that needs to reach something real gets an instant `ConnectionRefusedError`, not a timeout — mock the seam instead, or bind to `127.0.0.1` (loopback is allowed, which is how the aiosmtpd tests work). This is what keeps the suite at ~1 min 50 s; before it, ten tests waiting on timeouts were 75% of the runtime.
- `themis/services/tasks.py` defines its **own** `TaskStatus` enum — distinct from `taskqueue.TaskStatus`. Don't conflate them.
- API version is config-driven: `create_app()` reads it via `CR.get_app_version()` from `appVersion` in `SecOpsConfig.json` (currently `0.5.10`) — it is not hardcoded.
- `features.themis.areLocalIpsAllowed` is set to `true` in `SecOpsConfig.json` (intentional, for local dev against private IPs). **Must be reverted to `false` before any real deployment**, or the anti-SSRF defense stays disabled in production. The suite does *not* warn you if you forget: the three SSRF tests (`test_nikto_rejects_loopback_target`, `test_nikto_rejects_cloud_metadata_target`, `test_nmap_rejects_private_ip_target`) pin the flag to `false` themselves through an autouse fixture in `test_themis.py`, so they pass either way and prove nothing about the deployed value.
